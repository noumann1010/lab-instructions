...
The datasaurus_dozen dataset contains 1846 rows and 3 columns.
...
```{r star-data, eval = FALSE}
star_data %
  filter(dataset == ___)
```

**Step 2: Create the plot**
```{r star-plot, fig.fullwidth=TRUE, eval = FALSE}
ggplot(data = ___, mapping = aes(x = x, y = y)) +
  geom_point()
```
...
- The correlation coefficient for the star dataset is: 

- Compared to the dino dataset (r = -0.06447185), the star dataset's correlation is: 
...
- The correlation coefficient for the circle dataset is: 

- Compared to the dino dataset, the circle dataset's correlation is:
